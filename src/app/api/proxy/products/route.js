...
// Fallback products – live demo (Vercel) er jonno
...
export async function GET() {
  try {
    // 🔹 Local development: Express backend thakle eta use hobe
    ...
    console.error("Proxy GET /products error, using fallback:", err);

    // 🔹 Backend na thakle (Vercel e) – fallback data
    return new Response(JSON.stringify({ products: fallbackProducts }), {
      status: 200,
      headers: { "Content-Type": "application/json" },
    });
  }
...
// POST ta mainly local backend er jonno – same moto thakte ditechhi
